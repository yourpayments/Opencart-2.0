...
class ControllerPaymentPayU extends Controller {
	public function index() {
		...
		$option  = array(   'merchant' => $this->config->get('payu_merchant'), 
							...
							'button' => '<input type="submit" value="'.$data['button_confirm'].'" class="btn btn-primary" style="float: right;" />' );
...
//		$data['continue'] = $this->url->link('checkout/success');
		...
		//		$this->render();
		//		catalog\view\theme\default\template\payment\payu.tpl
	}
...
#======================= IPN READ ANSWER ============================
	...
	{	
		$arr = &$this->dataArr;
		$arr = $_POST;
		foreach ( $this->IPNcell as $name ) if ( !isset( $arr[ $name ] ) ) die( "Incorrect data $name" );
